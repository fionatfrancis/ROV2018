...
``` {r summary-stats, warnings = FALSE}
...
TRANSECT WIDTHS TO CALCULATE FISH DENSITIES
...
fish.density <- all.data %>% group_by(Survey.abbrv, Species, RCA, NAME, AREA, simple.sub, simple.bio) %>% summarise(total.fish = sum(Count), transect.area = mean(transect.area)) %>%
...
percent.sub <- substrate %>% group_by(Survey.abbrv) %>% count(sub.types)
...
percent.sub %>% group_by(sub.types) %>% summarise(mean.percent = mean(percent), sd.percent = sd(percent))
